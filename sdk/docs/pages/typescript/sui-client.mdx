...
with four local validators, a Full node, and a faucet server. Note that the local network will not be persisted when passing the `--dont-persist-state` flag.
Refer to [the Local Network guide](https://docs.sui.io/guides/developer/getting-started/local-network) for more information.
