...
spin up a local network with four local validators, a Full node, and a faucet server. Note that the
local network will not be persisted when passing the `--random-genesis` flag. Refer to
