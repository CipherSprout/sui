...
//# run-graphql
{   # Go from one version to another using atVersion
    v1: package(address: "@{P1}") {
        v1: atVersion(version: 1) {
            module(name: "m") {
                functions { nodes { name } }
            }
        }
        v2: atVersion(version: 2) {
            module(name: "m") {
                functions { nodes { name } }
            }
        }
        v3: atVersion(version: 3) {
            module(name: "m") {
                functions { nodes { name } }
            }
        }
...
{   # Fetch out of range versions (should return null)
    ...
    v1: package(address: "@{P0}") {
        v0: atVersion(version: 0) {
            module(name: "m") {
                functions { nodes { name } }
            }
        }

        v4: atVersion(version: 4) {
            module(name: "m") {
                functions { nodes { name } }
            }
        }
