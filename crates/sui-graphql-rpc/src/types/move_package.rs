...
impl MovePackage {
    ...
    async fn at_version(&self, ctx: &Context<'_>, version: u64) -> Result<Option<MovePackage>> {
        MovePackage::query(
            ctx,
            self.super_.address,
    ...
    async fn latest(&self, ctx: &Context<'_>) -> Result<MovePackage> {
...
impl MovePackage {
    ...
    pub(crate) async fn paginate_by_checkpoint(
        ...
        let before_checkpoint =
            (checkpoint_viewed_at + 1).min(before_checkpoint.unwrap_or(u64::MAX));
    ...
    pub(crate) fn try_from_stored_history_object(
        ...
        let object = Object::try_from_stored_history_object(history_object, checkpoint_viewed_at)?;
