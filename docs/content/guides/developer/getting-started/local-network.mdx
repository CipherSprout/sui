...
* calls the `sui start` command, passing two flags `--with-faucet` to start a faucet service, and `--random-genesis` to generate a new genesis and not persist the local network
...
**Important:** Each time you start the network by passing `--random-genesis`, the network starts as a new network with no previous data. The local network is not persistent.
